            ...
            if unread_email:
                ...
                with allure.step(f'Обработка письма от {sender} с темой "{subject}"'):
                    ...
                    if sender in accept_mail_list:
                        ...
                        print(f"Обнаружено непрочитанное письмо, отправитель {sender} отсутствует в accept_mail_list, ответ не отправлен. Письмо помечено как непрочитанное.")
                        mail_client.mark_mail_as_unread()
                        allure.attach(f'Ответ не отправлен: {sender} отсутствует в списке разрешенных. Письмо возвращено в непрочитанные.', 'Статус ответа')
